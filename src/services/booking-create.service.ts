...
async function findOrCreateWalkInCustomer(name: string, phone: string): Promise<number> {
...
async function checkTimeConflict(
  ...
  const barberConflict = await prisma.booking.findFirst({
    where: {
      barberId,
      bookingDate,
  ...
  if (barberConflict) throw new Error('Barber is not available at this time');
...
async function validateAndCalculateDiscount(promoCode: string, servicePrice: number) {
...
export async function createBooking(data: {
  ...
  if (isRandom) {
    const { getRandomBarber } = await import('./barber.service');
    const randomBarber = await getRandomBarber(data.bookingDate);
    if (!randomBarber) throw new Error('No available barbers for this date');
  ...
  if (!finalCustomerId) throw new Error('Customer information is required');
  ...
  const { getBookingById } = await import('./booking-query.service');
